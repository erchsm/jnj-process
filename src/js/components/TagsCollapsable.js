...
export default class TagsCollapsable extends Component {
	...
	render () {
		...
		return tags.length ?
			<div className='tags-collapsable__wrapper'>
				...
				<ul className='tags-collapsable__list tags-wrapper' ref={(_ref) => this.tagsRef = _ref}>
					{tags}
					{showMoreNumber > 0 && !isOpened ?
						<li key={`tag-show-more-number`}>
							<a onClick={this.toggle} className='tags-collapsable__show-more-number'>
								{showMoreNumber}+
							</a>
						</li>
						: null
				...
				{showMoreNumber > 0 ?
					(!isOpened ?
						<a onClick={this.toggle} className='tags-collapsable__show-more'>
							<span className='tags-collapsable__more-text'>Show All Topics</span>
							<span className='iconcss icon-caret-down-lg'></span>
						</a> :
						<a onClick={this.toggle}  className='tags-collapsable__show-less'>
